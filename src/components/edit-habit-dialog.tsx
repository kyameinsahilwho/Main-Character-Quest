...
export function EditHabitDialog({ children, habit, onUpdateHabit }: EditHabitDialogProps) {
  ...
  const [frequency, setFrequency] = useState<'daily' | 'weekly' | 'monthly' | 'every_2_days' | 'every_3_days' | 'every_4_days'>(habit.frequency);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[90vw] max-w-[600px] bg-white border-2 border-b-8 border-[#CBD5E1] text-[#1E293B] rounded-[2rem] shadow-2xl p-0 overflow-hidden flex flex-col max-h-[90vh]">
        <DialogTitle className="sr-only">Edit Ritual</DialogTitle>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E2E8F0] bg-white flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#F1F4F9] border-2 border-b-4 border-[#CBD5E1] flex items-center justify-center">
              <Edit2 className="w-5 h-5 text-[#1E293B] stroke-[3]" />
            </div>
            <h2 className="text-lg font-black font-headline uppercase tracking-tight text-[#1E293B]">Edit Ritual</h2>
          </div>
        </div>
